...
class RegistroNombres:
        ...
        while len(lista_actual) < 20:
            try:
                with sr.Microphone() as source:
                    ...
                    # Escucha y reconoce el audio
                    audio = self.listener.listen(source)
                    texto = self.listener.recognize_google(audio, language='es-ES').lower()

                    # Agrega directamente el texto reconocido a la lista
    ...
    def generar_tuplas(self):
        ...
        for i in range(10000):
            ...
            edad = random.randint(1, 100)  # Edad entre 1 y 100 años
            peso = random.uniform(1.0, 100.0)  # Peso entre 1.0 y 100.0 kg
...
def contar_hermanos(tuplas):
    ...
    for tupla in tuplas:
        # Obtener apellidos paternos y maternos
        apellido_paterno = tupla[2]
        apellido_materno = tupla[3]
        nombre_completo = tupla[1]

        # Crear una clave en el diccionario para el par de apellidos
        clave = (apellido_paterno, apellido_materno)
        
        # Si no existe en el diccionario, lo inicializamos con una lista vacía
        if clave not in grupos_hermanos:
            grupos_hermanos[clave] = []

        # Agregar el nombre a la lista correspondiente al par de apellidos
        grupos_hermanos[clave].append(nombre_completo)

    # Contar a los hermanos: si hay más de una persona con los mismos apellidos, cuentan como hermanos
    for grupo, nombres in grupos_hermanos.items():
        if len(nombres) > 1:
            contador += len(nombres)  # Sumar el número de personas que tienen los mismos apellidos
...
def generar_histograma_edades(tuplas):
    edades = [tupla[2] for tupla in tuplas]
    
    localidades = ["Barcelona", "Madrid", "Sevilla", "Valencia", "Zaragoza"]
    edades_por_localidad = {localidad: [] for localidad in localidades}
...
#Genere el histograma de pesos (total y por localidad--5)
...
def main():
    ...
    for tupla in datos[:10]:  # Muestra 10 tuplas
        print(tupla)
